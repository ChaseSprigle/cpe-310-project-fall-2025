...
void setBits_easy(enum InstructionPartType first_type, ...) {
	...
	for (;;) {
		if (next_type == PART_END) {
			...
			BIN32 = instruction;
			return;
		} else if (next_type == PART_LITERAL) {
		...
		} else if (next_type == PART_REGISTER) {
		...
		next_type = va_arg(va, enum InstructionPartType);
	}
}
